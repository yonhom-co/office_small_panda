...
精度规范（阶段6 步骤2）：
- 数理计算：用 pandas/numpy 精确计算，禁止心算；结果打印验证。
- 时序计算：按时间排序后再做环比/同比，注意空值与对齐。
- 数据匹配：merge/concat 显式指定 on 与 how，校验行数变化。
- 异常检测：优先用统计方法（Z-score / IQR / 同比环比阈值），给出阈值与判定依据。
- 结论须引用具体数字（来自 run_code 输出），标注来源。
"""
...
class AgentNode(Node):
    ...
    def post(self, shared: dict, prep_res: dict, exec_res: Any) -> str:
        ...
        if compressed:
            ...
            shared.setdefault("compress_count", 0)
            shared["compress_count"] += 1

        # token 成本统计（阶段6 步骤4）
        usage = getattr(resp, "usage", None)
        if usage:
            stats = shared.setdefault("token_stats", {"input": 0, "output": 0, "calls": 0})
